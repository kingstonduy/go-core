...
func TestRunner(t *testing.T) {
	...
	t.Run("A=request-response", func(t *testing.T) {
		...
		test.Test_Should_Return_Both_Response_And_Error()
	})
...
func (t *MediatRTests) Test_RegisterRequestHandlerFactory_Should_Return_Error_If_Handler_Already_Registered_For_Request() {
	...
	expectedErr := fmt.Sprintf("registered handler already exists in the registry for message %s", "*pipeline.RequestTest")
...
func (t *MediatRTests) Test_Should_Return_Both_Response_And_Error() {
	handler4 := &RequestTestHandler4{}
	err := RegisterRequestHandler[*RequestTest4, *ResponseTest4](handler4)
	require.NoError(t, err)

	req := RequestTest4{Data: "test"}
	res, err := Send[*RequestTest4, *ResponseTest4](context.Background(), &req)

	assert.NotNil(t.T, res, err)
	assert.Equal(t.T, res.Data, req.Data)
	assert.Equal(t.T, err.Error(), "failed to handle request")
...
type RequestTest4 struct {
	Data string
}

type ResponseTest4 struct {
	Data string
}

type RequestTestHandler4 struct {
}

func (c *RequestTestHandler4) Handle(ctx context.Context, request *RequestTest4) (*ResponseTest4, error) {
	fmt.Println("RequestTestHandler4.Handled")
	testData = append(testData, "RequestTestHandler4")

	return &ResponseTest4{Data: request.Data}, fmt.Errorf("failed to handle request")
}
