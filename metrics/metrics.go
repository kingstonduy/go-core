...
// UpdateFilter overwrites the existing filter with the given rules.
func (m *Metrics) UpdateFilterAndLabelsNotOverride(allow, block, allowedLabels, blockedLabels []string) {
	m.UpdateFilterAndLabels(
		append(m.AllowedPrefixes, allow...),
		append(m.BlockedPrefixes, block...),
		append(m.AllowedLabels, allowedLabels...),
		append(m.BlockedLabels, blockedLabels...),
	)
}
