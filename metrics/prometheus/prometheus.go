...
const (
	DefaultPrometheusSink = "default_mcs_prometheus_sink"
)
...
var (
	...
	DefaultPrometheusOpts = PrometheusOpts{
		...
		Name:       DefaultPrometheusSink,
	}
	DefaultSumaryObjectives = map[float64]float64{0: 0, 0.25: 0.05, 0.5: 0.05, 0.75: 0.05, 0.9: 0.01, 0.99: 0.001}
)
...
func initSummaries(m *sync.Map, summaries []SummaryDefinition, help map[string]string) {
	for _, s := range summaries {
		...
		pS := prometheus.NewSummary(prometheus.SummaryOpts{
			...
			Objectives:  DefaultSumaryObjectives,
		})
